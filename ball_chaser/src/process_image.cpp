...
// The PImage_node class contains a call back function that continuously executes and reads the image and loop through each pixel looking for a value equal to 255, if so it drives the motor: forward, left or right. This class also includes a drive_robot function that calls the command_robot service to drive the robot in some specied direction. 
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub1 = n.subscribe("/camera/rgb/image_raw", 10, &PImage_node::process_image_callback, &Listener);
